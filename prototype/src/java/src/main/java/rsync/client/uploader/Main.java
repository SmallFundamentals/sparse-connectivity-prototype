...
public class Main {
    ...
    public static void main(String[] args) throws IOException, NoSuchAlgorithmException, DigestException {
        String partialFileName = args[0];
        String originalFilePath = args[1];

        System.out.println(String.format("UPLOADING for test - '%s'", partialFileName));
        System.out.println();

        ChecksumResult cr = NetworkUtil.getChecksumResult(originalFilePath, partialFileName);

        BufferedInputStream dataStream;
        try {
            dataStream = new BufferedInputStream(new FileInputStream(originalFilePath));
